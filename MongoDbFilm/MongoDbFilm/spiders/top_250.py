...
try:
    os.remove("MongoDbFilm/MongoDbFilm/spiders/top_250_serie.csv")
    print("top_250_serie.csv supprimé.")
except:
    print('Pas de fichier série csv.')



max_scraping_film = 5
max_scraping_serie = 5
...
class Top250Spider(scrapy.Spider):
    ...
    def parse(self, response):
        for result in response.css('tr > td.titleColumn')[:max_scraping_film]:
...
class Top250SpiderSerie(scrapy.Spider):
    ...
    def parse(self, response):
        for result in response.css('tr > td.titleColumn')[:max_scraping_serie]:
            yield scrapy.Request(url= self.base_url + result.css('a::attr(href)').get(), callback=self.parse_detail)
